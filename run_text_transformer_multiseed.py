...
class TextTransformerRegressor(nn.Module):
    def __init__(
        self,
        input_dim,
        model_dim=MODEL_DIM,
        ...
        self.position_embedding = nn.Parameter(torch.zeros(1, max_seq_len, model_dim))
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run Text-Transformer baseline on MOSI.")
...
def main():
    ...
    print("Experiment: Text-Transformer strong unimodal baseline")
    print("Feature file:", feature_path)
    print("Seeds:", args.seeds)
    print("Model: input projection + Transformer encoder + mean pooling + MLP regressor")

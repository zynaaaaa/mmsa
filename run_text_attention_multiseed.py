...
class TextBiLSTMAttentionRegressor(nn.Module):
    ...
    def forward(self, x):
        ...
        pooled = torch.sum(outputs * attention_weights, dim=1)
...
def run_one_seed(data, seed):
    ...
    model = TextBiLSTMAttentionRegressor(input_dim=x_train.shape[2]).to(device)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run Text-BiLSTM-Attention baseline on MOSI.")
...
def main():
    ...
    print("Model: BiLSTM + attention pooling + MLP regressor")
